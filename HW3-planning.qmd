...
ggplot(data = fish_result, aes(x = sample_date, y = location, fill = max_catch_fam)) +
  geom_bar(stat = "identity") +
   ...
   scale_x_date(expand = as.Date(c('2002','2025')),
                date_breaks = "1 year",
             limits = as.Date(c('02-01-2022', '07-31-2024')),
                labels = year
             ) +
...
fish_unique <- fish_clean |> 
  ...
  mutate(location = fct_reorder(location, nunique))




water_plot <- ggplot(data = fish_unique, aes(x = location, y = nunique)) +
  geom_col(fill = "navy", width = 1) +
  geom_text(aes(label = nunique, 
                vjust = 4), 
            color = "white", 
            size = 5,
            family = "outfit") +
  geom_vline(xintercept = c(1,2,3,4) , color = "blue3", linetype = "dashed") +
  theme_bw() +
  scale_x_discrete(expand = c(0, NA)) +
...
ggsave(
  filename = here::here("images", "water_bar_factor.pdf"),
  plot = water_plot,
  device = "pdf",
  width = 14, 
  height = 12,
...
ggplot(data = local_join, aes(y = latitude, x = longitude, color = avg_water_temp)) +
  ...
  scale_color_viridis_c(name = "Avg. Water Temp." , option = "plasma") +
labs(
