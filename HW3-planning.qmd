...
# mutate(sample_date = as.Date(sample_date, format = "%Y-%d-%m"))
   mutate(sample_date = parse_date_time(sample_date, orders = c("mdy", "dmy", "ymd", "my", "dmy"))) |> 
...
ggplot(data = fish_result, aes(x = location, y = sample_date, fill = max_catch_fam)) +
  ...
  scale_y_date(expand = c(0, NA), date_breaks = "2 months") +
  labs(
...
### code not working, attempt a reprex 
